...
#define DEFURASSR_BDS 0.5            /* default accurary of ssr corr (m) */
                  ...
                  int iode, double *rs, double *dts, double *var, int *svh)
{
    ...
    *svh=-1;
    ...
    if (sys==SYS_GPS||sys==SYS_GAL||sys==SYS_QZS||sys==SYS_CMP) {
        ...
        eph2pos(time,eph,rs,dts,var);//计算卫星位置
                    ...
                    int ephopt, double *rs, double *dts, double *var, int *svh)
{
    ...
    for (i=0;i<n&&i<MAXOBS;i++) {
        ...
        if (!satpos(time[i],teph,obs[i].sat,ephopt,nav,rs+i*6,dts+i*2,var+i,  //rs 卫星位置和速度 dts 钟差和钟漂
                    svh+i)) {
