...
const prcopt_t prcopt_default = { /* defaults processing options */
    //PMODE_PPP_KINEMA,0,2, SYS_GPS ,   /* mode,soltype,nf,navsys */   /* positioning mode (PMODE_???) */ /* positioning mode (PMODE_???) */ /* number of frequencies (1:L1,2:L1+L2,3:L1+L2+L5) */ /* navigation system */
    PMODE_PPP_KINEMA,0,2,SYS_GPS|SYS_CMP,   /* mode,soltype,nf,navsys */
	GNSISB_CT,
	7.0*D2R,{{0,0}},           /* elmin,snrmask */      /* elevation mask angle (rad) *//* SNR mask */
    3,4,0,0,                    /* sateph,modear,glomodear,bdsmodear *///ck     /* satellite ephemeris/clock (EPHOPT_???) */  /* AR mode (0:off,1:continuous,2:instantaneous,3:fix and hold,4:ppp-ar) */ /* GLONASS AR mode (0:off,1:on,2:auto cal,3:ext cal) */ /* BeiDou AR mode (0:off,1:on) */
	...
	//update
	{1E-7,4.0E-2,1E-4,1E-1,1E-2,0.0}, /* prn[] */    /* process-noise std [0]bias,[1]iono [2]trop [3]acch [4]accv [5] pos */
               ...
               double *Q)
{
    ...
    if (m<n) return -1;
    Ay=mat(n,1);
    matmul("NN",n,1,m,1.0,A,y,0.0,Ay); /* Ay=A*y */   //input a=7*12 y=12*1  NN:a*y  T��ʾ*ת��
	...
	double *xp, double *Pp)
{
	...
	matmul("TN", m, m, n, 1.0, H, F, 1.0, Q);       //Q=H'*F+R            /* Q=H'*P*H+R */
													...
													//Pp=(I-K*H')*P*(I-K*H')'+K*R*K' ��ʽ�� Pp=(I-K*H')*P ���Ƚ�
	if (!(info = matinv(Q, m))) {
		matmul("NN", n, m, m, 1.0, F, Q, 0.0, K);   /* K=P*H*Q^-1 */
		matmul("NN", n, 1, m, 1.0, K, v, 1.0, xp);  /* xp=x+K*v */
...
extern int filter(double *x, double *P, const double *H, const double *v, double *R, int n, int m, int *nn, double *std)
{
	...
	ix = imat(n, 1);
	for (i = k = 0; i < n; i++)
		if (x[i] != 0.0&&P[i + i * n] > 0.0) ix[k++] = i;//�ҳ�״̬����δ֪����������,Kδ֪��������=λ��+�Ӳ�+tro+ion+���Ǹ���

	x_ = mat(k, 1); xp_ = mat(k, 1); P_ = mat(k, k); Pp_ = mat(k, k); H_ = mat(k, m);
	for (i = 0; i < k; i++) {
		x_[i] = x[ix[i]];
		for (j = 0; j < k; j++) P_[i + j * k] = P[ix[i] + ix[j] * n];//155*155->n*n Ϊ
...
extern int readpcv(const char *file, pcvs_t *pcvs)
{
  ...
  //  
  //  if (!(ext=strrchr(file,'.'))) //strrchr() �����ҳ� str �ַ���������һ�γ��ֵ��ַ� c �ĵ�ַ��Ȼ�󽫸õ�ַ����
		//ext="";
  //  
...
{
    int i=(int)(lat/15.0);//С��15��
    if (i<1) 
		return coef[0];   
	else if (i>4)        //����75��
		return coef[4];
    return 
		coef[i-1]*(1.0-lat/15.0+i)+coef[i]*(lat/15.0-i); //�ڲ�
...
extern void antmodel_s(int sat, const pcv_t *pcv, double nadir, double *dant)
{
	...
	sys = satsys(sat, NULL);
	for (i = 0; i<NFREQ; i++) {
		//��interpvar��������nadirҲ������5.0��������ȷ������
		//����nadir*R2D��ֵ������14�����ڣ�
		//�ֱ�ʹ�����ַ�������alrtվ2010�������ݣ����ֳ���5.0�Ľ����̷߳���ƫ��ϵͳΪ����
		//������5.0�Ľ����̷߳�����ϵͳƫ��
